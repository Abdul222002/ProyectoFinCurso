...
@dataclass
class ScoringConfig:
    """Configuración flexible del sistema de puntuación"""
    ...
    # Bonus extras
    duels_won_per_point: int = 6
    accurate_passes_per_point: int = 30
...
class FantasyScoringEngine:
    ...
    def __init__(self, config: Optional[ScoringConfig] = None):
        ...
        logger.info("Motor de puntuación inicializado")
    ...
    def calculate_points(self, stats: PlayerStats) -> PlayerStats:
        ...
        if rating_points:
            breakdown['Nota del partido'] = rating_points
    ...
    def _calculate_rating(self, stats: PlayerStats) -> float:
        """Puntos por nota del partido (escala lineal)"""
        if not stats.rating:
            return 0
    ...
    def _calculate_defense_bonus(self, stats: PlayerStats) -> float:
        ...
        points += math.floor(stats.tackles / self.config.tackles_per_point)
        points += math.floor(stats.interceptions / self.config.interceptions_per_point)
        return points
    ...
    def _calculate_extra_bonus(self, stats: PlayerStats) -> float:
        """Bonus y penalizaciones extras"""
        points = 0
        points += math.floor(stats.duels_won / self.config.duels_won_per_point)
        points += math.floor(stats.accurate_passes / self.config.accurate_passes_per_point)
    ...
    def _calculate_penalties(self, stats: PlayerStats) -> float:
        ...
        points = 0
        # Estos pueden ajustarse según reglas del juego
        # Por ahora solo como placeholder si los datos están disponibles
        if hasattr(stats, 'penalty_save') and stats.penalty_save:
            points += stats.penalty_save * 5  # Bonus por parar penalti
        if hasattr(stats, 'penalty_miss') and stats.penalty_miss:
            points -= stats.penalty_miss * 2  # Penalización por fallar
        return points
...
class ResultsPresenter:
    ...
    def print_match_header(home_team: str, away_team: str, home_score: int, away_score: int):
        ...
        print(f"\n{'='*130}")
        print(f"⚽ SISTEMA DE PUNTUACIÓN FANTASY FOOTBALL")
        print(f"{'='*130}")
    ...
    @staticmethod
    def print_top_scorers(players_stats: List[PlayerStats], top_n: int = 5):
...
def analyze_fixture(fixture_id: int, config: Optional[ScoringConfig] = None, 
    ...
    logger.info(f"Análisis completado - {len(players_stats)} jugadores procesados")
    print("="*130 + "\n")
...
def test_custom_config():
    ...
    custom_config = ScoringConfig(
        goals_fwd=5,  # Aumentar puntos para delanteros
        assist_goal=4,  # Aumentar valor de asistencias
        rating_max_points=5,  # Más puntos por buena nota
    )
    
    analyze_fixture(19428171, config=custom_config, show_detailed=True)


def batch_analyze_fixtures(fixture_ids: List[int]):
...
if __name__ == "__main__":
    # Análisis simple
    analyze_fixture(19428171, show_detailed=True, top_n=10)
    
    # Análisis con configuración personalizada
    # test_custom_config()
    
    # Análisis por lotes
    # batch_analyze_fixtures([19428171, 19428172, 19428173])
