...
def _send_smtp_email(to_email: str, username: str, code: str):
    """Sends the OTP code via SMTP with timeout (called in background thread)"""
    ...
    try:
        # Timeout de 10 segundos para evitar bloqueo infinito
...
def _send_resend_email(to_email: str, username: str, code: str):
    """Sends the OTP code via Resend API (fast, no blocking)"""
    if not RESEND_API_KEY:
        print("WARNING: Resend API key not set.")
        return
    ...
    try:
        resp = requests.post(
            ...
            json={
                "from": f"SPL Game <onboarding@resend.dev>",
                "to": [to_email],
                "subject": subject,
                "html": html,
            },
        ...
        if resp.status_code == 200:
            print(f"✅ Email de verificación enviado a {to_email} via Resend")
        else:
...
def send_verification_email(to_email: str, username: str, code: str):
    """Sends the OTP code. Uses Resend if available, otherwise SMTP in background thread."""
    if RESEND_API_KEY:
        # Resend es más rápido y no se bloquea
        _send_resend_email(to_email, username, code)
    else:
        # SMTP en hilo separado para no bloquear la respuesta
        t = threading.Thread(target=_send_smtp_email, args=(to_email, username, code), daemon=True)
        t.start()
