...
// ── PARTICLES CANVAS (Subtle Gold Dust) ──────────────────────────────────────
function Particles() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        const a = (Math.sin(t+p.phase)+1)/2*0.4+0.1;
        ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2);
        ctx.fillStyle = `rgba(201,168,76,${a})`; ctx.fill(); // Gold
      });
...
function TiltCard({ children, className, onClick }) {
  ...
  const onMove = useCallback(e => {
    ...
    ref.current.style.transform = `perspective(700px) rotateX(${y}deg) rotateY(${x}deg) translateY(-4px) scale(1.02)`;
      ...
      <div className="ar">
        <div className="ar-content">
          ...
          <div className="ar-hero">
            <div className="ar-badge">TEMPORADA 25–26</div>
            <div className="ar-title">
              FOOT<span className="ar-hl" data-t="ARENA">ARENA</span>
            </div>
            <div className="ar-sub">Elige · Combate · Domina</div>

            <div className="lg-card ar-stats">
              {[
                { l:"ELO",      v:<Counter value={status?.global_elo || 0}/>,     cls:"sg", pre:"⚔️ " },
                { l:"Tickets",  v:<Counter value={status?.arena_tickets || 0}/>, cls:"sb", pre:"🎟 " },
                { l:"V",v:<Counter value={historyWins}/>,    cls:"sg", pre:"" },
                { l:"E", v:<Counter value={historyDraws}/>,    cls:"sc", pre:"" },
          ...
          <div className="lg-tabs" style={{ marginBottom: '24px' }}>
            {[["battle","⚔️ Combate"],["history","📋 Historial"],["rankings","🏅 Rankings"]].map(([id,lbl])=>(
              <button key={id} className={`lg-tab ${tab===id?"active":""}`} onClick={()=>setTab(id)}>{lbl}</button>
            ))}
          </div>
          ...
          {tab === "battle" && <>
            ...
            {teams.length === 0 ? (
                ...
                {teams.map(t => (
                    <TiltCard
                    key={t.id}
                    className={`ar-tc lg-card ${myTeam?.id===t.id?" sel":""}`}
                    ...
                    <div className="tc-orow">
                        <span className="tc-num">OVR {t.overall_rating?.toFixed(0)}</span>
                    </div>
                    </TiltCard>
                ))}
                </div>
          ...
          {tab === "history" && <>
            ...
            {history.length === 0
              ? <div className="lg-empty"><p>Aún no has jugado ningún partido</p></div>
              : <div className="ar-hlist">
                {history.map((h,i) => {
                    const eloVal = h.global_rating_change ?? 0;
                    const diff = h.result === 'draw'
                      ? '= ELO'
                      : `${eloVal >= 0 ? '+' : ''}${eloVal}`;
                    return (
          ...
          {tab === "rankings" && <>
            <div className="ar-sub2">Top 50 jugadores globales</div>
